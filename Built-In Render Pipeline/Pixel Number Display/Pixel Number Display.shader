Shader "PinkRammy/Unlit/PixelNumberDisplay"
{
    Properties
    {
        _BackgroundColor ("Background Color", Color) = (0, 0, 0, 1)
        _ValueColor ("Value Color", Color) = (1, 1, 1, 1)
        _Value ("Value (0 - 9999)", Range(0, 9999)) = 1337
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            */
            ...
            float4 getDigitCoords(uint digits)
            {
                ...
                if (digits == 2)
                {
                    return float4(0.3, 0.7, 0.0, 0.0);
                }

                // For 3 digits, we draw in a UV space split in three
                if (digits == 3)
                {
                    return float4(0.25, 0.5, 0.75, 0.0);
                }
            ...
            float getDigitCenterX(uint digits, uint index)
            {
                ...
                {
                    return getDigitCoords(4)[index];
                }

                float4 digitCoords = getDigitCoords(digits);
            ...
            float drawNumber(float2 uv, uint value)
            {
                ...
                float cellSize = digitsSameSize ? 0.025 : 0.1 / digits;
                float result = 0.0;
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                uint number = clamp(_Value, 0, 9999);
